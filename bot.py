...
# I totally accidentally left my API key here.

# Wit API Centric Functions
def get_intent(resp):
    '''Takes in resp for user's input. Processes resp to return the intent. If
    no intent is found, returns None.'''
    try:
...
def get_entities(resp):
    '''Takes in resp for the response from Wit.ai. Processes resp to return the
    list of entities from the response from Wit.ai.'''
...
def replace_char(st, o, n):
    '''Takes in st for string, o for character(s) to be replaced, and n for
    character(s) to replace o. Processes the string such that o is replaced by n
    in the string.'''
    end = ''
    tem = st.split(o)
...
def remove_quotes(st):
    '''Takes in st for string. Removes the apostrophes and/or quotation marks in
    an inputted string.'''
    st = replace_char(st, '\'', '"')
    st = replace_char(st, '"', '')
    return st
...
def write_to_log(to_log):
    '''Takes in to_log for response of the chatbot. Logs the datetime in which
    the response was given and the response itself, separated by a newline. Then
    inserts two newlines after the response.'''
...
def chat(inp):
    '''Takes in inp for user's input from the interface. The input is then
    passed through the various chatbot functions to remove apostrophes and
    quotation marks from the input to prevent error then to get the response
    from Wit.ai, which is logged. Then the response from Wit.ai is further
    processed by other functions to return a polished response to the user in
    English.'''
...
def form_response(resp):
    '''Takes in resp for response from Wit.ai. The input is then passed through
    the various chatbot functions to return an English statement based on the
    response by Wit.ai.
    
    Note that the responses returned by form_response_(intent) is a tuple with
    two elements.

    The first element returned is a status number, used for context. 0 means
    that the context is correctly understood. 1 means that the context is
    misunderstood, and necessary misunderstanding handler functions must be
    invoked to understand the context fully.

    The second element returned is the text to be put up in the chatbot as a
    response by the chatbot.'''
...
def message_intent_misunderstood():
    '''Asks the user to repeat his/her input if chatbot misunderstands intent of
    the earlier input.'''
    digit = randrange(3)
    if digit == 0:
...
def context_misunderstood(context):
    '''Takes in the context for context in question. Appends the context to the
    context in question.'''
...
def change_last_intent(intent):
    '''Takes in intent for the latest intent used. Clears the last_intent then
    appends intent to last_intent.'''
    last_intent.clear()
